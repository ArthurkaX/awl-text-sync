...
from .encoding import write_cp1252_text, write_utf8_text
...
WORKSPACE_RULES = """# Rules For Editing This Project
...
def write_cp1252_crlf(path: Path, text: str) -> None:
    ensure_directory(path.parent)
    write_cp1252_text(path, text)
...
def ensure_workspace_gitignore(workspace_root: Path) -> None:
    ...
    if gitignore_path.exists():
        return
    gitignore_path.write_text(WORKSPACE_GITIGNORE.replace("\n", "\r\n"), encoding="utf-8", newline="")


def ensure_workspace_rules(workspace_root: Path) -> None:
    rules_path = workspace_root / "docs" / "working_rules.md"
    if rules_path.exists():
        return
    ensure_directory(rules_path.parent)
    rules_path.write_text(WORKSPACE_RULES.replace("\n", "\r\n"), encoding="utf-8", newline="")
